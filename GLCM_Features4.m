...
% checked 
out.autoc = zeros(size_glcm_3,1); % Autocorrelation: [2] 
out.contr = zeros(size_glcm_3,1); % Contrast: matlab/[1,2]
out.corrm = zeros(size_glcm_3,1); % Correlation: matlab
out.corrp = zeros(size_glcm_3,1); % Correlation: [1,2]
out.cprom = zeros(size_glcm_3,1); % Cluster Prominence: [2]
out.cshad = zeros(size_glcm_3,1); % Cluster Shade: [2]
out.dissi = zeros(size_glcm_3,1); % Dissimilarity: [2]
out.energ = zeros(size_glcm_3,1); % Energy: matlab / [1,2]
out.entro = zeros(size_glcm_3,1); % Entropy: [2]
out.homom = zeros(size_glcm_3,1); % Homogeneity: matlab
out.homop = zeros(size_glcm_3,1); % Homogeneity: [2]
out.maxpr = zeros(size_glcm_3,1); % Maximum probability: [2]

out.sosvh = zeros(size_glcm_3,1); % Sum of sqaures: Variance [1]
out.savgh = zeros(size_glcm_3,1); % Sum average [1]
out.svarh = zeros(size_glcm_3,1); % Sum variance [1]
out.senth = zeros(size_glcm_3,1); % Sum entropy [1]
out.dvarh = zeros(size_glcm_3,1); % Difference variance [4]
%out.dvarh2 = zeros(size_glcm_3,1); % Difference variance [1]
out.denth = zeros(size_glcm_3,1); % Difference entropy [1]
out.inf1h = zeros(size_glcm_3,1); % Information measure of correlation1 [1]
out.inf2h = zeros(size_glcm_3,1); % Informaiton measure of correlation2 [1]
%out.mxcch = zeros(size_glcm_3,1);% maximal correlation coefficient [1]
%out.invdc = zeros(size_glcm_3,1);% Inverse difference (INV) is homom [3]
out.indnc = zeros(size_glcm_3,1); % Inverse difference normalized (INN) [3]
out.idmnc = zeros(size_glcm_3,1); % Inverse difference moment normalized [3]
